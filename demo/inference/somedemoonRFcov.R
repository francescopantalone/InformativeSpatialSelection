...
showMethods(classes=class(RMgauss()))

#RandomFields:::variance(RMgauss())


x=seq(0,5,length.out=101);
y<-RFcov(model = RMgauss(), distances=x)
z<-2*RFcov(RMgauss(), distances=0)-2*y

plot(x,z,type='l')
plot(x,y,type='l')
set.seed(42)
g_m_y <- RFsimulate(model = RMgauss(var=5,scale=.15) + RMtrend(mean = 10), x = seq(0, 1, length = 50), y = seq(0, 1, length = 50))
x <- seq(0, 1, length = 200)
y <- seq(0, 1, length = 200)
x1 <- rep(x, length(y))
x2 <- rep(y, each = length(x))
grf <- data.frame(x1, x2, y = g_m_y@data[[1]])
z <- grf$y * 2 # proxy variable
grf <- cbind(grf, z)
grf_grid <- expand.grid(seq(0, 1, length = 20), seq(0, 1,length = 20)) # grid
colnames(grf_grid) <- c("x1", "x2")
grf %>% as.data.frame %>% 
  ggplot(aes(x1, x2)) + geom_point(aes(size = y), color = "blue", alpha = 3/4) + 
  ggtitle("Random field") + coord_equal() + theme_bw()
tikz("toto.tikz",width = 2,height=3)
...
X11(type="cairo")
plot(prc_y)
savePlot(filename = paste0("prc_y.", "png"))

X11(type="cairo")
plot(prc_z)
savePlot(filename = paste0("prc_z.", "png"))


X11(type="cairo")
tikz(file = "prc_y.tex", width = 2, height = 3)
plot(prc_y)
dev.off()

prc_grid <- expand.grid(seq(0, 1, length = 100), seq(0, 1, length = 100)) # grid
X11(type="cairo")
tikz(file = "prc_z.tex", width = 2, height = 3)
plot(prc_z)
points(prc_grid[pps_sample[1,], ], pch = "21")
dev.off()

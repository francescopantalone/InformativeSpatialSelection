...
library(tidyr)
# --------------------------------- #
# ------------ simulation random field with spherical covariance model ------------ #
set.seed(42)
g_m_y <- RFsimulate(model = RMspheric() + RMtrend(mean = 10), x = seq(0, 1, length = 20), y = seq(0, 1, length = 20))
x <- seq(0, 1, length = 20)
y <- seq(0, 1, length = 20)
x1 <- rep(x, length(y))
...
grf %>% as.data.frame %>% 
  ...
  ggtitle("Spherical covariance model") + coord_equal() + theme_bw()
# --------------------------------------------------------------------------------- #
# ------------ draw samples ------------ #
npop <- nrow(grf)
nsample <- 150

...
for(i in 1:M)
{
  fit_vgm_srs[[i]] <- fit.variogram.reml(log(zinc) ~ 1, meuse[s_srs[i,], ], model = vgm(1, "Gau", 300, 1))
  fit_vgm_pps[[i]] <- fit.variogram.reml(log(zinc) ~ 1, meuse[s_pps[i,], ], model = vgm(1, "Gau", 300, 1))
}
# ------------------- #
# --------------------------------- #
# ------------ plot ------------ #

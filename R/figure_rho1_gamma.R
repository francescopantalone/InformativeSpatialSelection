#' Generates the rho_{1} function of the paper, when \eqn{gamma} is varying
#' 
#' This function 
#'  - generates the rho_{1} function of the paper, when \eqn{gamma} is varying. 
...
figure_rho1_beta <- function(pdfexportfolder = NULL, pngconvert = FALSE){
  ...
  figureXX <- ggplot(data = subset(rho_1, .Beta == 1 & y %in% yplot ),
  ...
  if(!is.null(pdfexportfolder)){    
    figbasename=paste0("figure_rho1_gamma.pdf")
    ...
    fs::file_show(file.path(getwd(),"latex/fig/figure_rho1_gamma.pdf"))
    if(pngconvert){
      temppngfile<-tempfile(pattern = ".png")
      pdftools::pdf_convert(output, "png", dpi = 1200,filenames = temppngfile)
      file.copy(from = temppngfile, to = file.path(pdfexportfolder, "figure_rho1_gamma.png"), overwrite = TRUE)
    }
  }
  figureXX
}
